    ...
    namespace Misc {
        ...
        public:
            void Game_Msg(const char* msg) {
                m_pCvar->ConsoleColorPrintf(Color(200, 255, 0, 255), "[gamesense] ");
...
GuiContext* majorkadev::Globals::Gui_Ctx = NULL;
...
std::string HWNDToString(HWND inputA)
{
	...
	{
		s.resize(len + 1);
		len = GetWindowTextA(inputA, LPSTR(&s[0]), s.size());
		s.resize(len);
	}
	return s;
}
...
void ui::GetInputFromWindow(const std::string& window_name) {	
	...
	ScreenToClient(FindWindowA("Valve001", nullptr), &p_mouse_pos);
...
void ui::Slider(const char* label, T* v, T v_min, T v_max, const char* format, GuiFlags flags, float scale, int remove) {
	...
	Render::Draw->Text(formatted_string, slider_bb.Min.x + rect_width - value_width.x / 2, slider_bb.Min.y, LEFT, Render::Fonts::Tahombd, true, D3DCOLOR_RGBA(255, 255, 255, std::min(200, g.MenuAlpha)));
}
