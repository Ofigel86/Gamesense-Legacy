...
BOOL ConfigManager::DirectoryExists( LPCTSTR szPath ) { // -@majorkadev
...
std::vector<std::string> ConfigManager::GetConfigs( ) {
	...
	if( created_cfg ) {
		...
		std::wstring str = full_path.wstring( ) + XorStr( L"\\skeet" );
	...
	std::string config_extension = XorStr( ".skeet" );
	std::vector<std::string> names;

	WIN32_FIND_DATAA find_data;
	HANDLE preset_file = FindFirstFileA( ( XorStr( "skeet\\cfg\\*" ) + config_extension ).c_str( ), &find_data );
	...
	if( preset_file != INVALID_HANDLE_VALUE ) {
		do {
			...
			// no.
			if( s == XorStr( "global_data.skeet" ) )
				continue;

			int pos = s.find( XorStr( ".skeet" ) );

			// NOT FOUND NIGGUH
			if( pos == -1 )
				continue;

			s.erase( s.begin( ) + pos, s.begin( ) + pos + 6 );
...
void ConfigManager::LoadConfig( std::string configname, bool load_user_data ) {
	std::ifstream input_file = std::ifstream( ( XorStr( "skeet\\cfg\\" ) + configname + XorStr( ".skeet" ) ).c_str( ) );
	if( !input_file.good( ) )
		return;
	...
	try {
		...
		// yes it is nigga
		auto parsed_config = nlohmann::json::parse( decoded_string.erase( 0, 10 ) );
...
void ConfigManager::SaveConfig( std::string configname, bool load_user_data ) {
	std::ofstream o( ( XorStr( "skeet\\cfg\\" ) + configname + XorStr( ".skeet" ) ).c_str( ) );
	if( !o.is_open( ) )
		return;
	...
	o.clear( );
	auto str = base64::encode( ( std::string( XorStr( "[skeet] - " ) ).append( g_Vars.m_json.dump( -1, '~', true ) ) ).c_str( ) );
	o << str;
	o.close( );
...
void ConfigManager::RemoveConfig( std::string configname ) {
	std::remove( ( XorStr( "skeet\\cfg\\" ) + configname + XorStr( ".skeet" ) ).c_str( ) );
}
...
void ConfigManager::OpenConfigFolder( ) {
	...
	std::wstring str = full_path.wstring( ) + XorStr( L"\\skeet\\cfg" );
